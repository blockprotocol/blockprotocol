...
export const getRandomOrganizationName = () => {
  const prefixIndex = Math.floor(Math.random() * organizationPrefixes.length);
  const suffixIndex = Math.floor(Math.random() * organizationSuffixes.length);

  return `${organizationPrefixes[prefixIndex]} ${organizationSuffixes[suffixIndex]}`;
};
