...
More information on the motivation can be found in [the specification overview.](/docs/spec)
...
Blocks can be [published on the Hub](/docs/developing-blocks#publishing) to make them browsable on [blockprotocol.org](/hub), and to make their metadata and source code available via our API.
...
We believe the best blocks will communicate data back and forth with the embedding application, making use of the [operations defined in the spec](/docs/spec/graph-service).
...
To save data beyond the session, blocks should make use of the [operations defined in the spec](/docs/spec/graph-service) to send updates to the embedding application.
...
Moving beyond a single set of constrained schemas that rarely change introduces a number of issues. However, we believe that all of these can be mitigated and that the benefits of an open ecosystem of well-described, discoverable, and end-user defineable types greatly outweighs any potential drawbacks.

  1. **Reusability of types:** the type system supports a high degree of composability, with data types and property types indvidually defineable and addressable from within different entity types and contexts.
  1. **Convergence on types:** the type editors developed by [HASH](https://hash.dev/) as part of their ontology manager include functionality to encourage users to where utilize and extend or otherwise crosswalk to existing types. These best-practice editors are open-source and are free to use within any other embedding application. We will be replacing the Block Protocol schema editor with these new type editors shortly.
