...
2.  Run `npx create-block-app your-block-name --template template`.
3.  Switch to your new folder: `cd [your-block-name]`.
4.  Run `yarn install && yarn dev`.
5.  Open [http://localhost:63212](http://localhost:63212/) in your browser to see your block.
...
1.  sets up message handling as described in [block-application communication](https://www.notion.so/Core-Specification-v0-2-c5a95a9542614d139e02443a5dad8cee).
1.  loads the block with the appropriate strategy for its `kind`.
...
1.  the `BlockEntityProperties` type in `src/app.tsx` defines the properties expected for the `blockEntity` (part of [the Graph Service](#reading-and-writing-data-with-the-graph-service)).
...
## Reading and writing data with the Graph Service
...
You can review every message defined in the graph specification [here](/spec/graph-service-specification). Bear in mind the following:
...
Once you've built a block, you can add it to the [Block Hub](/hub), so that:
...
1.  Claim your username by signing up at [https://blockprotocol.org](/)
...
3.  use a React component as a wrapper around something else. As an example, see [this blog post](https://hash.dev/blog/build-blocks-in-any-language) on writing a block in F#, wrapped as a React component.
