...
You can [see an example block using `updateEntities` here.](https://github.com/hashintel/hash/tree/main/packages/blocks/code)
...
- you can include dependencies in your block, but keep in mind bundle size. Common dependencies which you can reasonalby expect an embedding application to provide (e.g. React) can be defined as `externals`/`peerDependencies` - see [the template README](https://github.com/blockprotocol/blockprotocol/tree/main/packages/block-template).
