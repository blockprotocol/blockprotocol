...
The Graph Service is available via the `graphService` property in each starter template. It has a number of methods corresponding to the messages defined in the [specification](/docs/spec/graph-service).
...
You can also review the messages defined in the graph specification [here](/docs/spec/graph-service). If you do so, bear in mind the following:
