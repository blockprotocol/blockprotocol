...
The `@blockprotocol/core` package exports for embedding applications' convenience:
- a `renderHtmlBlock` function which handles both assigning an implementation of the `blockprotocol` object described below to the global scope, and rendering the block within a provided node
- an `assignBlockProtocolGlobals` function, which simply assigns the `blockprotocol` object to the global scope, in case the embedding application wishes to handle rendering the block separately.
...
- `"init"`: a message a block sends on load
...
**Messages** should be specified in a JSON file. The JSON file must conform to [the constraints defined by the Service Specification JSON Schema.](#json-schema-for-service-specification) To aid understanding, the constraints defined by the JSON schema are laid out in this section (**definition**), and a section related to **messages.**
