...
Blocks can be [published to the Block Hub](/docs/publishing-blocks) to make them browsable on [blockprotocol.org](/hub), and to make their metadata and source code available via our API.
...
To save data beyond the session, blocks should make use of the [operations defined in the spec](/spec/block-types#entity-functions) to send updates to the embedding application.
