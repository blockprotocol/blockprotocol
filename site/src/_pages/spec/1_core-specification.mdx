...
It defines how "services" should be specified. Services provide functionality by describing the messages blocks and applications may exchange.
...
**Service:** a description of the purpose and content of messages sent between blocks and applications, and any additional required metadata, defined according to this specification.
...
A block package might be made available via a URL, package manager (e.g. [npm](https://www.npmjs.com/)), or block catalog (e.g. [the Block Hub](https://blockprotocol.org/hub)).
...
- **a metadata file describing the block**, in JSON format, which:
  ...
  - MUST specify in all circumstances:
    ...
    - `type`: the type of block this is, so that embedding applications know how to render it – see [Types of block](#types-of-block)).
...
- where the block `entryPoint` is `"custom-element"`, MUST also specify a `tag` key, to specify the element tag name. This is important where the block’s code relies on the custom element being registered by the application using a specific tag.
...
4.  setting up message handling as described in [block-application communication](#block-application-communication)
...
- MUST dispatch events via a `CustomEvent` on the SAME element the block dispatches messages from (identified via listening for [the initial `"init"` message](#core-messages) and subsequently dispatching and listening on the target of that initial event)
...
The content of `data` and the permissible codes under `errors` are left to service specifications to define.
...
- MUST have a `detail` object which:
  ...
  - MUST include the `service` this message relates to (or `"core"` for messages defined in this document)
  ...
  - `errors` MUST be an array of objects, each of which MUST have a `code` and `message` – both strings – and MAY have `extensions`, an arbitrary JSON object defined by the service specification. Valid error codes are defined per message, in an `errorCodes` property in [the message definition](#messages-error-codes).
...
```json
{
  ...
  "properties": {
    ...
    "detail": {
      ...
      "required": ["requestId", "name", "service", "source"]
    }
  },
...
The `data` and `errors` of any messages exchanged between applications and blocks, and not described in [Core messages](#core-messages), MUST be described in separate [service specifications](#defining-services).
...
- `"initRequest"`: a message a block sends on load
- `"initResponse"`: a message the embedder sends in response

These messages are how blocks may exchange messages which are `sentOnInitiliazation`, which might be values available immediately on load, or configuration information, or anything else the service defines as useful for blocks and embedders to exchange on initialization.
...
Blocks MUST listen for all subsequent events on the element they dispatched this message from.

Embedding applications MUST listen for this event being dispatched from the block, by attaching an event listener to an element wrapping or at the root of the block.

Embedding applications MUST dispatch ALL subsequent events intended for the block on the event’s `target` element, including the `"initResponse"` message described immediately below.
...
The `data` for both `"initRequest"` and `"initResponse"` MUST be an object, where

- the keys of the object MUST be the names of services
- the values for those keys MUST be objects
- the keys for that object MUST match the `name` of `messages` for that service which are marked as `sentOnInitialization`, with the data as described in the relevant service specification
...
A service aims to provide a logical grouping of functionality and/or solve a specific problem related to block-application interaction.

A service definition must set out the **messages** that a block and application will exchange as part of the service.

**Messages** should be specified in a JSON file. The JSON file must conform to [the constraints defined by the Service Specification JSON Schema.](https://www.notion.so/Core-Specification-v0-2-c5a95a9542614d139e02443a5dad8cee) To aid understanding, the constraints defined by the JSON schema are laid out in this section (**definition**), and a section related to **messages.**

Services may also include an additional markdown file which provides further explanation and narrative, and imposes any additional requirements not captured via messages. This may, for example, specify additional metadata related to the service that can be expressed as part of a block package, whether in `block-metadata.json` or in an additional file or files.

#### <Hidden>Definition </Hidden>Specification

Each service:

- MUST define any messages it introduces in a JSON file conforming to [this schema](#json-schema-for-service-specification)
- MAY define an additional Markdown file which:
  ...
  - is OVERRULED by the JSON file in the event that there is any inconsistency between the two in how messages and metadata are defined

The JSON schema requires that a service specification is a JSON object which
...
The messages a service defines should be relevant to the delivery of the service.

Messages are listed under in the `messages` key of the JSON file which defines the service.

#### <Hidden>Messages </Hidden>Specification

A service specification SHOULD include any messages to be sent as part of the service under a `messages` key on the JSON file defining the service.
...
- `respondedToByMessage`: a `string`, which names another entry in `messages`. The presence of this indicates this message is part of a request/response pair
...
### JSON Schema for Service Specification
...
```json
{
  ...
  "properties": {
    ...
    "glossary": {
      "type": "array",
      "description": "Definitions for key terms and concepts used in the service",
      "items": {
        "type": "object",
        "properties": {
          "term": {
            "type": "string",
            "description": "The word or term to be defined."
          },
          "definition": {
            "type": "string",
            "description": "A short definition of the term. Markdown is permitted."
          }
        }
      },
      "overview": {
        "type": "string",
        "description": "A longer description of the service, providing any explanation that would help understand the intention, theme and concepts of the service. Markdown is permitted."
      },
      "messages": {
        "type": "array",
        ...
        "items": {
          "type": "object",
          "properties": {
            "//": "@todo insert properties listed above"
          },
      ...
      "metadata": {
        "type": "object",
        "description": "Additional metadata files and keys that the blocks may add to its block package and block-metadata.json, respectively, which are relevant to the service.",
        "properties": {
          "files": {
            "type": "array",
            "description": "Additional metadata files related to the service that the block may (or is required to) include in its block package, if it uses the service.",
            "items": {
              "type": "object",
              "properties": {
                "description": {
                  "description": "A description of the purpose of the file. Markdown is permitted.",
                  "type": "string"
                },
                "filename": {
                  "description": "The name of the file, including its extension, as a string literal or Regular Expression",
                  "type": "string"
                },
                "required": {
                  "default": false,
          ...
          "keys": {
            "type": "array",
            "description": "The keys that blocks may (or are required to) add to block-metadata.json, related to this service",
            "items": {
              "type": "object",
              "properties": {
                "name": {
                  "description": "The purpose of this key. Markdown is permitted.",
                  "type": "string"
                },
                "type": {
                  "description": "The permitted types of value(s) for this key, expressed in JSON schema.",
