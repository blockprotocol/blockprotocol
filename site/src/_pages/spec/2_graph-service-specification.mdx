...
- MUST be represented by an object, which:
  - MUST contain an `entityTypeId` identifying the type, which MUST be a str
...
Blocks must pass all of the **identifying fields** provided to them when making requests in respect of any entity.

If the embedding application provides a value for any identifying field, it should be assumed it is required to reliably identify the entity.
...
- <span id="block-graph">`blockGraph`</span>: the entity graph with the block at
  its root, which contains: 
  - `linkedEntities`: the entities linked from the block entity, and from those entities, and so on, resolved to a depth determined by the embedding application 
  - `linkGroups`: the links from the block entity and the links from any entities in `linkedEntities`, grouped by source entity and link path
...
See in the repo [here](https://github.com/blockprotocol/blockprotocol/blob/main/packages/%40blockprotocol/graph/src/graph-service.json#L5)
