...
Each entity provided under `linkedEntities` or `linkedAggregations` will also have [identifying fields](#fields-identifying-entities) to be used as arguments when updating those entities.
...
**TypeScript types for these functions are available from the [`blockprotocol`](https://github.com/blockprotocol/blockprotocol/tree/main/packages/blockprotocol) package**
...
A `BlockProtocolEntity` contains [identifying fields](#fields-identifying-entities) and the entity's other properties.
...
- MUST contain:
  ...
  - `destinationEntityId` \[_string_] – the id of a single entity the link is made to
...
- MAY contain
  ...
  - `index` \[_integer_]\[_optional_]: the position of this link in a list (for where ordering of links is important).
...
(and the entities they link onwards to, depending on the depth the graph is resolved to from the starting entity). e.g. in the above example, the data for entity `company1` will be provided in the `linkedEntities` array, rather than injected into the properties of the user.

Links themselves – **edges** in the graph – will be provided to a block under a `linkGroups` field, which is an array of objects,
each of which groups links by their source entity and path (field name).
...
- `linkId` \[_string_]: the link to delete.
...
#### Linked Aggregations (links to aggregations)
...
**Example.** creating a `LinkedAggregationDefinition` with the following data indicates that this particular table should be linked to the top 10 sales by value, and that the link is conceptually made on the table’s `rows` field:
...
An entry in linkedAggregations is a `LinkedAggregation`, which adds the resolved `results` to the `LinkedAggregationDefinition`,
where the results follow the shape of the [`AggregateEntitiesResult` object](#aggregate-entities-result).
...
**accepts:** a single _array_ of _objects_ (`CreateLinkedAggregationAction`) -
each object is a `LinkedAggregationDefinition`, without `aggregationId`.

**returns:** the created `LinkedAggregationDefinition`s, i.e. _objects_ inside an _array_ (now with `aggregationId`)

```block-function
updateLinkedAggregation(actions: UpdateLinkedAggregationAction[]): Promise<LinkedAggregationDefinition[]>
```

updates one or more `LinkedAggregationDefinition`s.

**accepts:** a single _array_ of _objects_ (`UpdateLinkedAggregationAction`), each with the following shape:

- `aggregationId` \[_string_]: the id of the link to update.
- `data` \[_object_]: the [_operation_](#aggregate-entities-operation) to overwrite the existing one with.

**returns:** the updated`LinkedAggregationDefinition`s, i.e. _objects_ inside an _array_.

```block-function
deleteLinkedAggregation(actions: DeleteLinkedAggregationAction[]): Promise<boolean[]>
```

deletes one or more `LinkedAggregationDefinition`s.

**accepts:** a single _array_ of _objects_ (`DeleteLinkedAggregationAction`), each with the following shape:

- `sourceAccountId?` \[_string_]\[_optional_]: the `accountId` of the source entity.
- `aggregationId` \[_string_]: the id of the aggregation to delete.
...
retrieve one or more `LinkedAggregation`s.

**returns:** the retrieved `LinkedAggregations`, i.e. _objects_ inside an _array_. **Note:** this SHOULD include the `results` of the aggregation.

**accepts:** a single _array_ of _objects_ (`GetLinkedAggregationAction`), each with the following shape:

- `sourceAccountId?` \[_string_]\[_optional_]: the `accountId` of the source entity.
- `aggregationId` \[_string_]: the `LinkedAggregation` to retrieve.
