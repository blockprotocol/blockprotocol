...
- **source file or files** (e.g. HTML and JavaScript files), which
  ...
  - where HTML is included, SHOULD be valid HTML
...
- a JSON file containing **metadata describing the block**, which:
  ...
  - MUST specify:
    ...
    - `protocol`: the applicable block protocol version, once the protocol is versioned (it is currently not).
...
- `accountId`: an identifier the account/namespace/user to which the entity belongs
...
- implementing a readonly / display mode for data which they are unable to edit
- hiding or disabling specific controls depending on the specific permissions granted them

<Frame emoji="🤔">

There are various ways in which we may change the approach described here:

1.  Finer-grained updates to entities - rather than sending data objects with multiple keys and values as described below - would be useful in supporting an embedding application implement collaborative editing and undo/redo stacks.
...
- `operation?` <a id="aggregate-entities-operation" /> \[_object_]\[_optional_]: a description of the aggregation operation, which contains at least one of the following fields:
  - `entityTypeId` \[_string_]\[_optional_]: the type of entity to provide aggregated data for
  - `entityTypeVersionId` \[_string_]\[_optional_]: the type of entity to provide aggregated data for, limited to a specific version of the type
...
- MUST contain:
  ...
  - EITHER:
    ...
    - `aggregate` – an aggregation operation which the embedding application SHOULD resolve the link to, following the
- MAY contain

  - `sourceEntityAccountId` \[_string_]: the `accountId` of the source entity.
  - `sourceEntityTypeId`: \[_string_]: the `entityTypeId` of the source entity.
  - `sourceEntityVersionId` \[_string_]: optionally specify that this link is only from a specific version of the source entity.

- if `destinationEntityId` is defined, MAY contain:-
  - `destinationEntityAccountId` \[_string_]: the `accountId` of the destination entity.
  - `destinationEntityTypeId`: \[_string_]: the `entityTypeId` of the destination entity.
  - `destinationEntityVersionId` to pin the link to a specific version of the destination entity.
...
**E.g.1.** creating a `Link` with the following data indicates that this particular user should be linked to a company with id `company1`, and that the link conceptually is made on the user’s `employer` field – although when delivering data to blocks the resolved data for entity `company1` will be provided _separately_ to `user1`, in the `linkedEntities` array, rather than injected into the properties of the user, and the link itself available in the `linkGroups` array provided to the block.
...
Links to specific entities will be provided to a block under a `linkGroups` field, which is an array of objects,
...
The entities linked to will be provided under `linkedEntities`
...
- `sourceAccountId` \[_string_]: the `accountId` of the source entity.
- `sourceEntityId` \[_string_]: the `entityId` of the source entity.
