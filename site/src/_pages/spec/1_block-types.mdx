...
- `filters` \[_array_]: any filters applied (empty if none):
  ...
  - `operator` \[_enum_]: the filter operator. One of IS, IS_NOT, CONTAINS, DOES_NOT_CONTAIN, STARTS_WITH, ENDS_WITH, IS_EMPTY, IS_NOT_EMPTY.
...
**arguments:** optionally accepts a single _object_ with the
following shape
...
- `operation?` \[_object_]\[_optional_]: a description of the aggregation operation, which contains at least one of the following fields:
...
- `
    ...
    Promise<Link>[]>
  ` \ creates one or more links. \**returns:** the created links,
  i.e. _objects_ inside an _array_ (now with `linkId`)
- `updateLink(actions: Payload[]): Promise<Link>[]>` \ updates one
  or more links. \**returns:** the updated links, i.e. _
    objects
  _ inside an _array_ \`
    Payload**:**
  `accepts a single _array_ of _objects,_ each with the following
  shape: * `linkId` _[string]_: the id of the link to update. * `
    data
...
- `deleteLink(actions: Payload[]): Promise<boolean[]>` \ deletes one
  or more links. \**returns:** an _array_ of `
    boolean
...
- `getLink(actions: Payload[]): Promise<Link[]>` \ retrieve one or
  more links. \**returns:** the retrieved links, i.e. _
    objects
  _ inside an _array_. \
  `
    Payload**:**
  `
  accepts a single _array_ of _objects_ each with the following shape:
  * `linkId` _[string]_: the link to retrieve.

**Describing _links in JSON schema__
