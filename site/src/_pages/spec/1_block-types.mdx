...
An entry in linkedAggregations follows the shape of the [`AggregateEntitiesResult` object](#aggregation-result-object), with the addition of a `link` field containing the link which generated it (to allow identifying which entity and path the result ‘belongs’ to, where it might otherwise be ambiguous).
