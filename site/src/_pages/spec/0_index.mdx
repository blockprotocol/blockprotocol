...
**Block type:** a definition and implementation - i.e. code - for a discrete component on a web page, which provides functionality for structured data (rendering it, editing it). A block type specifies the data it accepts via a schema in accordance with [the spec](/spec/embedding-application-implementation).
