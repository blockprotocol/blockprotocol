...
A schema might describe an entity which is linked to a specific block (e.g. the schema for a Table block type, which has properties such as `initialColumns`), or an entity which can be used across multiple blocks (e.g. the schema for a Person, Movie, or List).
