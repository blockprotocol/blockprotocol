...
The hook module is dependent on the [SemType specification](https://semtype.org/spec), as it relies on the existence of **entities** – use of SemType is a prerequisite for use of the hook module.
...
To request a view is rendered into a hook, a block MUST send a `hook` [message](https://blockprotocol.org/spec/core#glossary). The value for `hookId` MUST be `null` when a block first establishes a hook.
