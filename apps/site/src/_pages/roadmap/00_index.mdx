...
- **HASH launch:** as of late 2023, our core team are currently focused on readying the open-source [HASH](https://hash.ai/) application for production. HASH is the most advanced and complete implementation of the Block Protocol today. View its developer roadmap at [hash.dev/roadmap](https://hash.dev/roadmap) and the [update announcement](https://hash.dev/blog/tech-tree#roadmap-update).
- **SemType launch:** after a long period of parallel development in the HASH repository, the _graph module_ from version 0.3 of the Block Protocol specification has now been [fully separated out](https://hash.dev/blog/announcing-semtype) into a standalone project, _SemType_, which governs ontology and knowledge — type definitions (data types, property types, entity types) and the data that conforms to them — distinct from the block/embedder messages that blocks rely on, such as `createEntity` and `queryEntities`. The graph module's specification now lives at [semtype.org/spec](https://semtype.org/spec).
