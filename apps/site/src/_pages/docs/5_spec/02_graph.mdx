...
Blocks also include an `example-graph` file (either `example-graph.ts` or `example-graph.json` depending on approach) which defines example data that a block can use.
...
A block using the graph module MUST include an `example-graph.json` file alongside its metadata. The file's contents
