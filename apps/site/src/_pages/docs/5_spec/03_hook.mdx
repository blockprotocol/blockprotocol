...
This document (version 0.3) defines a **hook module** according to the terms set out in the [core specification](https://blockprotocol.org/docs/spec/core).

Under the hook module, a block can invite embedding application to take over the rendering of an element within the block (the **hook**), allowing embedding applications to provide users with application-specific **views** of specific data types without the block needing to know about them.

The module is designed to meet the following aims:
...
The module meets these aims by allowing blocks to delegate visualizing and editing functionality upwards to the embedding application.

The hook module is dependent on [the graph module](https://blockprotocol.org/docs/spec/graph-module), as it relies on the existence of **entities** – use of the graph module is a prerequisite for use of the hook module.
...
To request a view is rendered into a hook, a block MUST send a `hook` [message](https://www.notion.so/Hook-module-spec-1fcc185dc67c4ce7ace06878a144977d). The value for `hookId` MUST be `null` when a block first establishes a hook.
...
- it MUST:
  - render a view informed by the contents of the `hook` message:
    ...
    - using data stored at the specified `path` on the entity associated with the specified `entityId`
      - The path is expressed in its components as an array of strings or numbers, where strings are property accesses and numbers are array indices. e.g. `["friends", 1]`, `["address", "street"]`
...
There are only two messages in the hook module: the `hook` request from the block, and the `hookResponse` acknowledgement from the embedding application.

### <Hidden>Message definitions </Hidden>Specification

The messages available for exchange in the hook module are defined in the [module's JSON definition](https://github.com/blockprotocol/blockprotocol/blob/main/libs/%40blockprotocol/hook/src/hook-module.json#L5) and are also
