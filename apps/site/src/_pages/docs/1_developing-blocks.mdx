...
- `hasLeftEntity` - the edge that appears between a link entity and its left entity
- `hasRightEntity` - the edge that appears between a link entity and its right entity
...
For example, to retrieve the subgraph rooted at all entities with a property `https://blockprotocol.org/types/@blockprotocol/name/` with a value of `"Alice"` or `"Bob"`:

```js
const nameIsFilter = (value) => ({
  field: ["properties", "https://blockprotocol.org/types/@blockprotocol/name/"],
  operator: "EQUALS",
  value,
});
