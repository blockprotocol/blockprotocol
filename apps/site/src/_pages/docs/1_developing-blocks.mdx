...
1.  at any time after this initialization, the block may send further messages via a `Module` for specific purposes, such as reading and writing data within the embedding application.

The starter blocks created by `create-block-app` implement a simple example of this lifecycle:

1.  the `BlockEntityProperties` type in `src/app.tsx` defines the properties expected for the `blockEntity` (part of [the Graph Module](#using-the-graph-module)).
...
Depending on the template, you may need to check that the `graphModule` is available before using it (we will be removing the need for this shortly). These checks are omitted below.

### Updating the `blockEntity`

A common use for the Graph Module is to update the `blockEntity` to update the properties that are sent to the block (including when it is loaded again in the future).
...
You can also review the messages defined in the graph specification [here](https://blockprotocol.org/docs/spec/graph-module). If you do so, bear in mind the following:
...
- if a message has `respondedToBy`, it expects a response and the `graphModule` method will return a Promise which will resolve with the `{ data, errors }` object. If a message does not expect a response, the method will return void.
