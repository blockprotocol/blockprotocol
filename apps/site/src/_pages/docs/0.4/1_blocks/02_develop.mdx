...
This guide helps get you set up and introduces some of the key features of the protocol that deal with creating, reading and updating data records (or “entities”), whose shape is described by the [SemType spec](https://semtype.org/spec).
...
See [working with types](/docs/working-with-types) for more information on SemType, or jump straight to
...
for example as part of [working with structured data](#working-with-structured-data):
...
1.  the block receives the data for `blockEntitySubgraph` ([described below](#working-with-structured-data)):
...
The `graphModule` API describes how entities — whose shape is described by the [SemType spec](https://semtype.org/spec) — can be created, queried, updated, and linked together, including the block entity. It enables your block to create, read, update, and delete data in the embedding application.

It is exposed via the `graphModule` property in each starter template and has a number of methods that mirror the messages a block and embedding application exchange about structured data.
...
It contains data sent from the embedding application to the block related to its structured data. Importantly:
...
Many of the messages sent from the application to the block via `graphModule` return a `Subgraph`.
...
A common use for `graphModule` is to update the block entity – to update the properties that are sent to the block.
...
More information on building filters can be found in the documentation for the `queryEntities` method on the `@blockprotocol/graph` package.

#### Going further with `graphModule`
