...
We believe the best blocks will communicate data back and forth with the embedding application, making use of structured data described by the [SemType spec](https://semtype.org/spec).
...
To save data beyond the session, blocks should make use of the operations defined in the spec to send updates to the embedding application, with structured data shaped by the [SemType spec](https://semtype.org/spec).
...
- permission grants for blocks and block types, that allow for users to control the level and types of access blocks have (e.g. the ability to access entity data stored by the embedding application outside of the block's own entity) or access to other services.
...
1.  **Reusability of types:** [SemType](https://semtype.org/spec) supports a high degree of composability, with data types and property types indvidually definable and addressable from within different entity types and contexts.
