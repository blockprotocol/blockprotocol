...
When using the Block Protocol's [SemType spec](https://semtype.org/spec), data is shared between applications and blocks
...
The Block Protocol's [type system](https://semtype.org/spec) is designed so that types are composed of re-usable, sharable parts, which can be
