...
When using [SemType](https://semtype.org/spec), data is shared between applications and blocks
...
The system by which types are defined is [SemType](https://semtype.org/spec), a standalone specification that the Block Protocol depends on.

## The type system

SemType is designed so that types are composed of re-usable, sharable parts, which can be
referenced by other users and by other types.

SemType consists of three classes of elements:
...
The easiest way to get started is to create a free [app.hash.ai](https://app.hash.ai/) account and follow the instructions to create a new entity type. Any public types you create here will be published to the [SemType registry](https://semtype.org/types) and will be usable by Block Protocol embedders, as well as blocks themselves.
...
[SemType](https://semtype.org/spec) types are representable within TypeScript, and as such it is possible to create TypeScript representations of entity types.
...
- a field `targets` which is an object which is a mapping of files to the types that should be generated within them
  ...
  - values of `targets` are an array where the items are objects which satisfy the following:
    - contain the key `sourceTypeId` with a value of a SemType ID (e.g. `https://blockprotocol.org/@blockprotocol/types/entity-type/thing/v/2`), _or_
