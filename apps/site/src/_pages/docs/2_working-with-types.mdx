...
There is a provided `codegen` utility in the graph module package which provides an implementation of such generation.
...
The `block-scripts codegen` utility is configured through the `codegen` object in a block's `package.json`.
...
- contain the key `blockEntityType` with value `true`
- this says to generate types for the block entity type (given by the `blockEntityType` property in the `blockprotocol` object in `package.json`), alongside type aliases to identify it as such (e.g. `type BlockEntity = ...`)
