...
Blocks and types can be published and discovered on the [Þ Hub](https://blockprotocol.org/hub), an open-source registry that anybody can use. It can be accessed one of two ways:

1. **in-browser** by navigating to [`blockprotocol.org/hub`](https://blockprotocol.org/hub).
2. **programmatically** by generating a free API key. Either a user or an embedding application must provide this in order to query and load blocks from the Þ Hub.
