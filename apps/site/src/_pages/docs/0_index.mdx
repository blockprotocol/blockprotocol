...
- **programmatically** from within other applications: either a user or an embedding application must create a free API key in order to query and load blocks from the Þ Hub via its API

The Þ Hub provides a persistent, reliable, and open-source single point of reference for the discovery of new content. This means creators only need to publish once, while embedding applications can easily discover new blocks, types and endpoints, making them instantly available to their own users.
