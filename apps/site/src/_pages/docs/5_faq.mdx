...
In most applications, the users inserting new blocks generally aren't developers, which means 

To facilitate this, we recommend that embedders implement an allowlist (or at minimum a denylist) of blocks, or provide their users the option to do the same. Today you might choose to allow/deny blocks on the basis of their authorship/origins, or based on their _verification status_.
 
In the future we plan to:
- expose additional information such as block usage metadata
- building support for managing allow/denylists linked to individual API keys directly from the blockprotocol.org website and CLI.
