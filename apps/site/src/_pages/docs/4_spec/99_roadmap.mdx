...
- **Prototype implementations at every step.** Battle-testing the specification and proving (or disproving) ideas early saves time, and minimizes arguments about hypotheticals. Multiple diverse prototype environments ensure abstractions generalize across embedding applications, with fewer navel gazing committees.
...
- **Hosted services as a convenience.** While we provide hosted services such as the [Þ Hub](https://blockprotocol.org/hub), these are offered as a convenience, and complement the specification. Their use is not required to utilize the protocol, although they make the discovery of blocks, types, services, and hooks easier, while providing fallbacks that embedding applications can opt-in to use where blocks require things beyond their current capabilities.
