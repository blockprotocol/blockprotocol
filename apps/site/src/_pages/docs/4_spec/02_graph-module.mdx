...
- `default`: an **entity**, conforming to the block’s entity type, representing the default data that applications should provide when creating a block, unless &#40;a&#41; the block can handle being provided no data when first instantiated or &#40;b&#41; `variants` is provided &#40;see below&#41;

- `examples`: an array of objects conforming to the block's entity type which provide examples of the data that can be passed to it
...
Consumers can pass this object to the graph module package's standard library to benefit from implementations of common operations such as getting entities linked to a particular entity.
...
> **Note**: The `revisionId` will be used to identify different revisions of the same element within a single subgraph, this could be multiple versions of the same ontology type, or multiple revisions of an entity in a datastore that supports versioning
...
The `roots` _field_ of the subgraph is an array of **Vertex IDs** which identify the roots, and are JSON objects which satisfies the following schema:
...
That is to say, `subgraph.vertices[baseId][revisionId]` MUST be the vertex of the root.
