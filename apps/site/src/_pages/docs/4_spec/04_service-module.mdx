...
The module defines messages which correspond to specific requests available from those external services.

## Glossary

**Provider:** a company, organization or person that provides a service or services (e.g. Mapbox)

**Service:** a request which meets a specific functional goal, typically external to the embedding application and accessed via third-party API endpoint or integration (e.g. Retrieve Address).
