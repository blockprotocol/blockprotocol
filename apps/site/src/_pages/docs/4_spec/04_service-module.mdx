...
The service module specifies named messages from named service providers, and any messages exchanged between application and block should be consistent with the shape and intent of those messages.

How exactly the embedding application generates those messages is beyond the scope of the Block Protocol, so long as the schema and intent of the message as described in this specification are met.
