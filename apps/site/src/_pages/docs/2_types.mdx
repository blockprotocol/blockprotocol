...
The Block Protocol's [type system](https://blockprotocol.org/docs/spec/graph#the-type-system) is designed so that types are composed of re-usable, sharable parts, which can be
